...
TestSource.prototype.getTile = function(z, x, y, callback) {
    var key = [z,x,y].join('-');
    fs.readFile(__dirname + '/' + key + '-undecorated.vector.pbfz', function(err, zdata) {
...
tape('setup source directly', function(assert) {
    new TestSource(null, function(err, testSource) {
        ...
        new TileliveDecorator({key:'BoroCode', source: testSource}, function(err, source) {
            assert.ifError(err);
            source.getTile(14, 4831, 6159, function(err, tile, headers) {
                assert.ifError(err);
...
(function() {
var client = redis.createClient();
var cache = require('lru-cache')({ max: 1000 });
